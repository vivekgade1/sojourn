...
- **Map view (default) — the expedition map.** Each session is a dotted trail across warm paper; each **waypoint is one turn** (your prompt plus everything the agent did for it), sized by how much work happened, numbered in order, folding serpentine when a session runs long. Verified flags appear as red hazard badges on the waypoint, advisory as amber outlines, decisions/checkpoints as pennants, and the newest turn carries the "you are here" pin. Clicking a waypoint opens a drawer with the turn's steps as chips — click a chip to inspect that node. Turn grouping is chronological, so it stays readable even when parent links are missing.
